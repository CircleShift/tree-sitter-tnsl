...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_tnsl", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_tnsl": ["*.pyi", "py.typed"],
        "tree_sitter_tnsl.queries": ["*.scm"],
    },
    ext_package="tree_sitter_tnsl",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_tnsl/binding.c",
                "src/parser.c",
            ],
            define_macros=[
